...
class HalfCheetah(SharedMemoryClientEnv, RoboschoolMujocoXmlEnv):
    '''
    ...
    def __init__(self):
        RoboschoolMujocoXmlEnv.__init__(self, "half_cheetah.xml", "torso", action_dim=6, obs_dim=22)
    ...
    def robot_specific_reset(self):
        init_pos = [0.5, 0.5, 1]
        init_pos = init_pos + init_pos
        index = 0
        for j in self.ordered_joints:
            j.reset_current_position(self.np_random.uniform( low=-0.1, high=0.1 ), 0)
            #j.reset_current_position(init_pos[index], 0)
            index += 1
        self.feet = [self.parts[f] for f in self.foot_list]
        self.feet_contact = np.array([0.0 for f in self.foot_list], dtype=np.float32)
        self.scene.actor_introduce(self)
        self.initial_z = None
        self.pre_body_pose = self.robot_body.pose()
    ...
    def calc_state(self):
        ...
        self.body_vel = (np.array(body_pose.xyz()) - np.array(self.pre_body_pose.xyz()))/self.scene.dt
        self.pre_body_pose = body_pose

        more = np.array([
            z-self.initial_z,
            self.body_vel[0], self.body_vel[2],
            p], dtype=np.float32)
        return np.clip( np.concatenate([more] + [j] + [self.feet_contact]), -5, +5) # 4, 12, 2
        #modified by dobro
