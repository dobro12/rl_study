...
def visualize_loss(seed, epoch, value_loss, q1_loss, q2_loss, policy_loss, reward_sum):
    ...
    if(epoch==0):
        value_loss_plt = vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([value_loss]) ,env = 'sac_'+str(seed) ,opts=dict(title="value_loss"))
        q1_loss_plt = vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([q1_loss]) ,env = 'sac_'+str(seed), opts=dict(title="q1_loss"))
        q2_loss_plt = vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([q2_loss]) ,env = 'sac_'+str(seed), opts=dict(title="q2_loss"))
        policy_loss_plt = vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([policy_loss]) ,env = 'sac_'+str(seed), opts=dict(title="policy_loss"))
        total_reward_plt = vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([reward_sum]),env = 'sac_'+str(seed), opts = dict(title="total_reward"))
        
    else:
        vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([value_loss]) , win=value_loss_plt,env = 'sac_'+str(seed), update = "append")
        vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([q1_loss]) , win=q1_loss_plt,env = 'sac_'+str(seed), update = "append")
        vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([q2_loss]), win=q2_loss_plt,env = 'sac_'+str(seed), update = "append")
        vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([policy_loss]) ,win=policy_loss_plt,env = 'sac_'+str(seed), update = "append")
        vis.line(X=torch.Tensor([epoch]), Y=torch.Tensor([reward_sum]), win=total_reward_plt,env = 'sac_'+str(seed), update = "append")
...
print(total_reward)
for seed in range(5):
    agent_setting_parameters['seed'] = seed
    ...
    for i in range(epoch):
        ...
        for t in range(episode):
            ...
            if(torch.cuda.is_available()):
                ...
                mu = mu.detach().cpu()
                var = var.detach().cpu()
            next_obs, reward, done, _ = env.step(action) if len(action.shape)==1 else env.step(action.squeeze())
