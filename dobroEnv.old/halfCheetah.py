...
class HalfCheetah(object):
    def __init__( self, pybullet_client, useFixedBase=True, arg_parser=None):
        ...
        self.power = 0.9
        self.dict_coef = {b'bthigh':120.0, b'bshin':90.0, b'bfoot':60.0, b'fthigh':140.0, b'fshin':60.0, b'ffoot':30.0}
        self.action = []
        self.joints = []
        self.force_coef = []
        self.joint_info = []
        ...
        for j in range(self.pybullet_client.getNumJoints(self.sim_model)):
            ...
            if joint_info[2] == 0 and not b'ignore' in joint_info[1]:
                ...
                self.force_coef.append(self.dict_coef[joint_info[1]])
    ...
    def apply_action(self, action):
        ...
        for i,j in enumerate(self.joints):
            force = action[i]*self.force_coef[i]*self.power
